...
presidents = """George Washington,  Feb 22 1732,    Westmoreland Co. Va.,   Dec 14 1799,    Mount Vernon Va.
...
presidents.split("\n").each do |pres|
	pres = pres.split(",")
	born = pres[1][-4, 4].to_i
	died = pres[3][-4, 4].to_i
	died = Date.today.year if not died > 0
	age = died + 1 - born
	age.times { |x| years[born+x] += 1 }
end

years = years.sort_by { |key, val| val }
years.each do |year, val|
	puts "#{year}: #{val}"
end
